...
1. Убедитесь, что у вас установлен Go (версия 1.24.9 или выше)
...
CORS_ALLOWED_ORIGIN="http://localhost:3000" go run main.go
...
- Порт: 8080
- Хост: localhost
- CORS origin: `http://127.0.0.1:5500` (для Live Server в VSCode)
...
- **Значение по умолчанию**: `http://127.0.0.1:5500` (для Live Server в VSCode)
...
Для разрешения запросов с любого источника (только для разработки):
```bash
CORS_ALLOWED_ORIGIN="*" go run main.go
```
