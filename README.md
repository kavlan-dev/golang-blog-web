...
✅ **Конфигурируемость** - Настройка через конфигурационный файл
...
Проект использует JSON файлы конфигурации, расположенные в директории `config/`:

- `config/config.json` - основной конфигурационный файл
- `config/config.example.json` - пример конфигурации

### Запуск с кастомным конфигом

Вы можете указать путь до конфигурационного файла с помощью флага `-p`:

```bash
go run main.go -p config/prod_config.json
```
...
```json
{
    ...
    "admin": {
        ...
        "password": "$2a$14$N9qo8uLOickgx2ZMRZoMy.M5Q1N3H3J5H3J5H3J5H3J5H3J5H3J5H3J",
...
**Разработка (используя дефолтный конфиг)**:
```bash
go run main.go
```

**Production (используя кастомный конфиг)**:
```bash
go run main.go -p config/production.json
```

**Docker совместимость**:
```bash
docker run -v ./config:/app/config your-image -p /app/config/production.json
```
...
API поддерживает базовую аутентификацию. При запуске приложения создается администратор с данными для входа указанными в конфигурационном файле. Эндпоинт `/api/auth/register/` позволяет создать нового пользователя без прав администратора. для повышения прав воспользуйтесь эндпоинтом `/api/secure/users/{id}`
